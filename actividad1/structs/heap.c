 ...
 */
HeapElem* crear_heap_elem(Coord pos, int f, int g, int h, HeapElem* parent, int obstaculo){
...
void flotar_elemento(Heap* h, int indice){
    if(indice <= 1 || indice > h->fin) return;
...
void hundir_elemento(Heap* h, int indice){
    if(indice<1||indice*2>h->fin) return;
    int menorHijo = indice * 2;
    if (indice * 2 + 1 <= h->fin && h->buffer[indice * 2 + 1]->f < h->buffer[indice * 2]->f) {
