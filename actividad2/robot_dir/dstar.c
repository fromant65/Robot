...
void d_star_lite(DStarData * data, Robot * r, int sensorRange) {
  ...
  while (data->sStart->pos.x != data->sGoal->pos.x
         || data->sStart->pos.y != data->sGoal->pos.y) {
    Node *start = data->sStart;
    if (start->g >= INFINITY) {
...
void initialize_weights(DStarData * data, int N, int M) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < M; j++) {
      ...
      if (i == 0)
        data->weights[i][j][0] = INFINITY;
      if (j == M - 1)
        data->weights[i][j][1] = INFINITY;
      if (i == N - 1)
        data->weights[i][j][2] = INFINITY;
      if (j == 0)
        data->weights[i][j][3] = INFINITY;
    }
  }
}
