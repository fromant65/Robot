...
void process_floodfill(const char *directory) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    snprintf(source_path, sizeof(source_path), "%s/%s", directory,
             entry->d_name);
  ...
  // Calcular promedios
  double program_average =
      program_count >
      0 ? (double) program_total_length / (double) program_count : 0.0;
...
void process_bfs(const char *directory) {
  ...
  // Calcular promedios
  double bfs_average =
      bfs_count > 0 ? (double) bfs_total_length / (double) bfs_count : 0.0;
